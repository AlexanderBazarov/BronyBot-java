...
public class TextsLocalisationServiceTest {
	@Test
	void getLocalReturnsCorrectMessageWithoutArgs() {
		...
		TextKey key = mock(TextKey.class);
		...
		try (MockedStatic<ResourceBundle> bundleMock = mockStatic(ResourceBundle.class)) {
			...
			String message = TextLocalisationService.getLocal(key, testLocale);
	...
	void getLocalReturnsCorrectMessageWithArgs() {
		...
		try (MockedStatic<ResourceBundle> bundleMock = mockStatic(ResourceBundle.class)) {
			...
			String message = TextLocalisationService.getLocal(key, testLocale, "World");
	...
	void getLocalReturnsQuestionMarksWhenGotMissingKeyWithArgs() {
		...
		try (MockedStatic<ResourceBundle> bundleMock = mockStatic(ResourceBundle.class)) {
			...
			String message = TextLocalisationService.getLocal(key, testLocale, "Test");
			assertEquals("???", message);
		}
	}
